...
async function initApp() {
  ...
  button.addEventListener('click', async () => {
    try {
      ...
      // 選択範囲内で「親が選択されていない」または「親がない」ものをルートとする
      ...
      function traverse(node, currentPath) {
        // HTMLタグ除去とカンマのエスケープ
        const rawText = node.nodeView?.text || "";
      ...
      console.error(err);
      await miro.board.notifications.showError('エラーが発生しました: ' + err.message);
    }
  });
}
